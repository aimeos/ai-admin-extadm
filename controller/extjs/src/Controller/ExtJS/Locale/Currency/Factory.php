...
{
	/**
	 * Creates a new locale currency controller object.
	 *
	...
	{
		/** controller/extjs/locale/currency/name
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Locale\Currency\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\locale\Currency\Mycurrency
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/locale/currency/name = Mycurrency
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/locale/currency/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Locale\\Currency\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Locale\\Currency\\' . $name;
		 ...
		 * ("\Aimeos\Controller\ExtJS\Locale\Currency\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/locale/currency/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Locale\Currency\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
