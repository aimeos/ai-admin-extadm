...
/**
 * Attribute type controller factory.
 *
 * @package Controller
 * @subpackage ExtJS
 */
...
{
	/**
	 * Creates a new attribute type controller object.
	 *
	...
	{
		/** controller/extjs/attribute/type/name
		 * Class name of the used ExtJS attribute type controller implementation
		 *
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Attribute\Type\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Attribute\Type\Mytype
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/attribute/type/name = Mytype
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/attribute/type/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Attribute\\Type\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Attribute\\Type\\' . $name;
		 ...
		 *
		 *  controller/extjs/attribute/type/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see controller/extjs/attribute/type/decorators/local
		 */

		/** controller/extjs/attribute/type/decorators/global
		 * Adds a list of globally available decorators only to the attribute type ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/attribute/type/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/attribute/type/decorators/local
		 * Adds a list of local decorators only to the attribute type ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 * ("\Aimeos\Controller\ExtJS\Attribute\Type\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/attribute/type/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Attribute\Type\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
