...
{
	/**
	 * Creates a new catalog import text controller object.
	 *
	...
	{
		/** controller/extjs/catalog/import/text/name
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Catalog\Import\Text\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Catalog\Import\Text\Mytext
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/catalog/import/text/name = Mytext
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/catalog/import/text/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Catalog\\Import\\Text\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Catalog\\Import\\Text\\' . $name;
