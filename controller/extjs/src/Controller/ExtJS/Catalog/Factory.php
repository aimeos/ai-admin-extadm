...
{
	/**
	 * Creates a new catalog controller object.
	 *
	...
	{
		/** controller/extjs/catalog/name
		 * Class name of the used ExtJS catalog controller implementation
		 *
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Catalog\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Catalog\Mycatalog
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/catalog/name = Mycatalog
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/catalog/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Catalog\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Catalog\\' . $name;
		 ...
		 *
		 *  controller/extjs/catalog/decorators/excludes = array( 'decorator1' )
		 *
		 * This would remove the decorator named "decorator1" from the list of
		 * common decorators ("\Aimeos\Controller\ExtJS\Common\Decorator\*") added via
		 * "controller/extjs/common/decorators/default" for the catalog ExtJS controller.
		 *
		 ...
		 * @see controller/extjs/catalog/decorators/local
		 */

		/** controller/extjs/catalog/decorators/global
		 * Adds a list of globally available decorators only to the catalog ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/catalog/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/catalog/decorators/local
		 * Adds a list of local decorators only to the catalog ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 * ("\Aimeos\Controller\ExtJS\Catalog\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/catalog/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Catalog\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
