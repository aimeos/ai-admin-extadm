...
{
	/**
	 * Creates a new media controller object.
	 *
	...
	public static function createController( \Aimeos\MShop\Context\Item\Iface $context, $name = null )
	{
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Media\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Media\Mymedia
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/media/name = Mymedia
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/media/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Media\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Media\\' . $name;
		 ...
		 * ("\Aimeos\Controller\ExtJS\Media\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/media/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Media\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
