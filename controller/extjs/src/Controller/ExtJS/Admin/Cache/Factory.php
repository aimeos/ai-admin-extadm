...
{
	/**
	 * Creates a new admin cache controller object.
	 *
	...
	{
		/** controller/extjs/admin/cache/name
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Admin\Job\Mycache
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/admin/cache/name = Mycache
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/admin/cache/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Admin\\Cache\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Admin\\Cache\\' . $name;

		$controller = self::createControllerBase( $context, $classname, $iface );

		/** controller/extjs/admin/cache/decorators/excludes
		 * Excludes decorators added by the "common" option from the admin cache ExtJS controllers
		 *
		 ...
		 *
		 *  controller/extjs/admin/cache/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see controller/extjs/admin/cache/decorators/local
		 */

		/** controller/extjs/admin/cache/decorators/global
		 * Adds a list of globally available decorators only to the admin cache ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/admin/cache/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/admin/cache/decorators/local
		 * Adds a list of local decorators only to the admin cache ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 * ("\Aimeos\Controller\ExtJS\Admin\Cache\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/admin/cache/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Admin\Cache\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
