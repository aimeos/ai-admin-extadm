...
{
	/**
	 * Creates a new admin log controller object.
	 *
	...
	{
		/** controller/extjs/admin/log/name
		 * Class name of the used ExtJS admin log controller implementation
		 *
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Admin\Job\Mylog
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/admin/log/name = Mylog
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/admin/log/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Admin\\Log\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Admin\\Log\\' . $name;

		$controller = self::createControllerBase( $context, $classname, $iface );

		/** controller/extjs/admin/log/decorators/excludes
		 * Excludes decorators added by the "common" option from the admin log ExtJS controllers
		 *
		 ...
		 *
		 *  controller/extjs/admin/log/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see controller/extjs/admin/log/decorators/local
		 */

		/** controller/extjs/admin/log/decorators/global
		 * Adds a list of globally available decorators only to the admin log ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/admin/log/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/admin/log/decorators/local
		 * Adds a list of local decorators only to the admin log ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/admin/log/decorators/local = array( 'decorator2' )
		 *
