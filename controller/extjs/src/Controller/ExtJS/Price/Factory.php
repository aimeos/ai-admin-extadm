...
/**
 * ExtJS price controller factory.
 *
 * @package Controller
 * @subpackage ExtJS
 */
...
{
	/**
	 * Creates a new price controller object.
	 *
	...
	{
		/** controller/extjs/price/name
		 * Class name of the used ExtJS price controller implementation
		 *
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Price\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Price\Myprice
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/price/name = Myprice
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'controller/extjs/price/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Price\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Controller\\ExtJS\\Price\\' . $name;
		 ...
		 *
		 *  controller/extjs/price/decorators/excludes = array( 'decorator1' )
		 *
		 * This would remove the decorator named "decorator1" from the list of
		 * common decorators ("\Aimeos\Controller\ExtJS\Common\Decorator\*") added via
		 * "controller/extjs/common/decorators/default" for the price ExtJS controller.
		 *
		 ...
		 * @see controller/extjs/price/decorators/local
		 */

		/** controller/extjs/price/decorators/global
		 * Adds a list of globally available decorators only to the price ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 *
		 *  controller/extjs/price/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/price/decorators/local
		 * Adds a list of local decorators only to the price ExtJS controllers
		 *
		 * Decorators extend the functionality of a class by adding new aspects
		 * (e.g. log what is currently done), executing the methods of the underlying
		 * class only in certain conditions (e.g. only for logged in users) or
		 ...
		 * ("\Aimeos\Controller\ExtJS\Price\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/price/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Price\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
