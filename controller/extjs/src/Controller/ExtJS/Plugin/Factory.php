...
{
	/**
	 * Creates a new plugin controller object.
	 *
	...
	{
		/** controller/extjs/plugin/name
		 * Class name of the used ExtJS plugin controller implementation
		 *
		 ...
		 *
		 *  \Aimeos\Controller\ExtJS\Plugin\Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  \Aimeos\Controller\ExtJS\Plugin\Myplugin
		 *
		 * then you have to set the this configuration option:
		 *
		 *  controller/extjs/plugin/name = Myplugin
		 *
		...
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Controller\\ExtJS\\Plugin\\' . $name : '<not a string>';
		 ...
		 * ("\Aimeos\Controller\ExtJS\Plugin\Decorator\*") around the ExtJS controller.
		 *
		 *  controller/extjs/plugin/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "\Aimeos\Controller\ExtJS\Plugin\Decorator\Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
		 ...
		 * @see controller/extjs/plugin/decorators/global
		 */
		return self::addControllerDecorators( $context, $controller, 'plugin' );
	}
}
